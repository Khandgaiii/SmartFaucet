...
import { signInWithGoogle } from "@/lib/firebase/google-signin"
...
export function GoogleSignInButton({ disabled, onError }: Props) {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      if (!out.ok) {
        if ("cancelled" in out && out.cancelled) return
        onError(formatAuthError(out.error))
        return
      }
      if (out.usedRedirect) return
      router.replace("/")
    } catch (e: unknown) {
